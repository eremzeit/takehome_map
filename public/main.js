...
var GeoHash = function () {
  ...
  _createClass(GeoHash, [{
    key: 'get',
    value: function get(lat, lng) {
      var _Util$projectionCoord = Util.projectionCoords([lat, lng], this.scale),
          latIdx = _Util$projectionCoord.latIdx,
          lngIdx = _Util$projectionCoord.lngIdx;

      return this.getByIdx(latIdx, lngIdx);
    }
  }, {
    key: 'getByIdx',
    value: function getByIdx(latIdx, lngIdx) {
    ...
    key: 'appendByIdx',
    value: function appendByIdx(_item, latIdx, lngIdx) {
      this._touch(latIdx, lngIdx);
      this.lookup[latIdx][lngIdx].push(_item);
    }
  }, {
    key: 'replace',
    value: function replace(item, lat, lng) {
      this.clearByCoords(lat, lng);
      this.append(item, lat, lng);
    }
  }, {
    key: 'append',
    value: function append(item, lat, lng) {
      var _Util$projectionCoord2 = Util.projectionCoords([lat, lng], this.scale),
          latIdx = _Util$projectionCoord2.latIdx,
          lngIdx = _Util$projectionCoord2.lngIdx;
    ...
    key: 'clearByCoords',
    value: function clearByCoords(lat, lng) {
      var _Util$projectionCoord3 = Util.projectionCoords([lat, lng], this.scale),
          latIdx = _Util$projectionCoord3.latIdx,
          lngIdx = _Util$projectionCoord3.lngIdx;
    ...
    value: function remove(shape) {
      ...
      var _Util$projectionCoord4 = Util.projectionCoords([lat, lng], this.scale),
          latIdx = _Util$projectionCoord4.latIdx,
          lngIdx = _Util$projectionCoord4.lngIdx;

      this._touch(latIdx, lngIdx);
      this.lookup[latIdx][lngIdx] = _.filter(this.lookup[latIdx][lngIdx], function (_shape) {
        _this._compareShapes(shape, _shape);
      });
    }
    ...
    key: '_compareShapes',
    value: function _compareShapes(s1, s2) {
      return s1._id == s2._id;
    }
  }, {
    key: '_touch',
    value: function _touch(latIdx, lngIdx) {
    ...
    key: 'getIndexPairs',
    value: function getIndexPairs() {
      var _this2 = this;

      var pairs = [];
      _.each(_.keys(this.lookup), function (latIdx) {
        var byLngIdx = _this2.lookup[latIdx];

        _.each(_.keys(byLngIdx), function (lngIdx) {
    ...
    key: 'logicalIntersection',
    value: function logicalIntersection(hash2) {
    ...
    key: 'logicalDifference',
    value: function logicalDifference(hash2) {
      var pairs1 = _.sortBy(this.getIndexPairs(), function (x) {
        return x;
      });

      var res = [];
      _.each(pairs1, function (pair) {
    ...
    value: function all() {
      ...
      _.each(_.keys(this.lookup), function (latIdx) {
        var byLngIdx = _this3.lookup[latIdx];
        _.each(_.keys(byLngIdx), function (lngIdx) {
      ...
      return _.map(pairs, function (pair) {
        return _this3.getByIdx(pair[0], pair[1]);
      });
    }
  }]);
...
GeoHash.forEachCell = function (bounds, scale, fn) {
  ...
  var calls = [];
  for (var i = minLatIdx; i <= maxLatIdx; i++) {
    for (var j = minLngIdx; j <= maxLngIdx; j++) {
      calls.push([i, j]);
    }
  }
...
},{"./constants":1,"./util":6,"expect.js":9,"q":12,"underscore":13}],3:[function(require,module,exports){
'use strict';

var IntensityMap = require('./intensity_map');

window.initMap = function () {
  var map = new IntensityMap();
  map.init();
  window.map = map;
};

},{"./intensity_map":4}],4:[function(require,module,exports){
'use strict';
...
var ShapePool = require('./shape_pool');
var _ = require('underscore');
var Q = require('q');
var Util = require('./util');

var IntensityMap = function () {
  function IntensityMap() {
    _classCallCheck(this, IntensityMap);

    this.googleMap = null;
    this.shapePool = null;
  }

  _createClass(IntensityMap, [{
    key: 'init',
    value: function init() {
      var _this = this;
      ...
      this.googleMap.addListener('bounds_changed', function () {
        IntensityMap.mapUpdatedDebounced(_this, 'bounds');
      });

      this.googleMap.addListener('zoom_changed', function () {
        IntensityMap.zoomChangedDebounced(_this);
      });

      setTimeout(function () {
        IntensityMap.mapUpdatedDebounced(_this, null);
        _this.loadShapesInCurrentBounds({ updateScale: true });
      }, 1000);
    }
  }, {
    key: 'loadShapesInCurrentBounds',
    value: function loadShapesInCurrentBounds(options) {
      var _this2 = this;

      console.log('Beginning data load');
      options = options || {};

      //TODO when are we going to incorporate this?
      var dataLayerId = _.random(3);

      var bounds = this._getQueryBounds();

      var requestedScale = this._findScaleByMapZoom(this.googleMap.getZoom());

      console.log('Requesting scale ' + requestedScale + ' from ' + this.googleMap.getZoom());
      IntensityMap.fetchShapes(bounds, 0, requestedScale).then(function (shapes) {
        var scale = (shapes[0] || {}).scale;

        if (scale !== requestedScale) {
          console.warn('the server didnt give the scale requested');
        }
        console.log('RECEIVED SCALE: ', scale);

        if (options.updateScale) {
          _this2.shapePool.scaleUpdated(scale);
        }

        return _this2.shapePool.dataRefreshedWithBounds(shapes, bounds, scale);
      }).done();
    }
  }, {
    key: '_findScaleByMapZoom',
    value: function _findScaleByMapZoom(mapZoom) {
      return {
        1: 32,
        2: 32,
        3: 16,
    ...
    key: '_getQueryBounds',
    value: function _getQueryBounds() {
      var bounds = this.googleMap.getBounds().toJSON();

      //todo: uncomment the code below to increase the size of the queried area
      var hFudge = (bounds.east - bounds.west) * 1;
      var vFudge = (bounds.north - bounds.south) * 1;

      var east = Util.normalizeLng(bounds.east + hFudge);
      var west = Util.normalizeLng(bounds.west - hFudge);
      var north = Util.normalizeLat(bounds.north + vFudge);
      var south = Util.normalizeLat(bounds.south - vFudge);
...
IntensityMap.fetchShapes = function (bounds, dataLayerId, scale) {
  ...
  var queryPath = [Util.normalizeCoords(bounds.north, bounds.west), Util.normalizeCoords(bounds.north, bounds.east), Util.normalizeCoords(bounds.south, bounds.east), Util.normalizeCoords(bounds.south, bounds.west), Util.normalizeCoords(bounds.north, bounds.west)];
  ...
  $.get({
    ...
    error: function error(req, _status, err) {
...
},{"./shape_pool":5,"./util":6,"q":12,"underscore":13}],5:[function(require,module,exports){
'use strict';
...
var ShapePool = function () {
  function ShapePool(googleMap, options) {
  ...
  _createClass(ShapePool, [{
    key: 'scaleUpdated',
    value: function scaleUpdated(newScale) {
    ...
    key: 'dataRefreshedWithBounds',
    value: function dataRefreshedWithBounds(shapes, bounds, scale) {
      var _this = this;
      ...
      var _shapes$ = shapes[0],
          latSize = _shapes$.latSize,
          lngSize = _shapes$.lngSize;


      var newItemsHash = new GeoHash(scale);
      shapes.forEach(function (shape) {
        newItemsHash.append(shape, shape.center[0], shape.center[1]);
      });

      // find the ones that need to be removed
      var indexPairs = this.getHash(scale).logicalDifference(newItemsHash);
      var toRemove = _.chain(indexPairs).map(function (pair) {
        return _this.getHash(scale).getByIdx(pair[0], pair[1]);
      }).value();
      ...
      return this.update(shapes, {}, scale).then(function () {
        return _this.remove(toRemove, scale);
      });
    ...
    key: 'update',
    value: function update(shapes, renderOptions, scale) {
      var _this2 = this;
      ...
      var chunks = Util.makeChunks(shapes, 5000);
      renderOptions = renderOptions || {};

      // Each do each chunk in a separate job
      var promises = _.map(chunks, function (shapes) {
        return Q.delay(0).then(function () {
          for (var j = 0; j < shapes.length; j++) {
            _this2.insertShape(shapes[j], scale, renderOptions);
          }
        });
      });
    ...
    key: 'insertShape',
    value: function insertShape(shape, scale, renderOptions) {
      var _this3 = this;
      ...
      var path = _.map(shape.loc.coordinates, function (p) {
        return Util.fromPointArrayToLatLngLiteral(p);
      });
      var options = Object.assign({}, Constants.DEFAULT_SHAPE_OPTIONS, renderOptions, {
        path: path,
      ...
      var toUpdate = _.first(this.getHash(scale).get(shape.center[0], shape.center[1]).slice());

      var oldGoogleMapObject = void 0;
      if (toUpdate) {
      ...
      if (oldGoogleMapObject) {
        ...
        var gShape = new google.maps.Polygon(options);
      ...
      this.allAtLatLng(shape.center[0], shape.center[1]).forEach(function (shape) {
        if (!toUpdate || shape._id != toUpdate._id) {
          _this3.remove(shape, shape.scale);
        }
      });
    ...
    key: 'remove',
    value: function remove(shapes, scale) {
      var _this4 = this;
      ...
      shapes.forEach(function (shape) {
        ...
        _this4.getHash(scale).remove(shape, scale);
      });
    ...
    key: 'clearAllShapes',
    value: function clearAllShapes(scale) {
      var _this5 = this;

      var shapes = this.allShapes();

      _.chain(shapes).each(function (s) {
        _this5.remove(s, s.scale);
      });
    }
  }, {
    key: 'getHash',
    value: function getHash(scale) {
    ...
    key: 'allShapes',
    value: function allShapes() {
      var r = _.chain(this.__geoHashByScale).values().map(function (geoHash) {
        return geoHash.all();
      }).flatten().value();
    ...
    key: 'latLngDistance',
    value: function latLngDistance(point1, point2) {
      if (!point1 || !point2) {
        throw new Error('invalid params');
      }

      var aSqrd = Math.pow(point2[0] - point1[0], 2);
      var bSqrd = Math.pow(point2[1] - point1[1], 2);
    ...
    key: 'allAtLatLng',
    value: function allAtLatLng(lat, lng) {
      return _.chain(this.__geoHashByScale).pairs().map(function (pair) {
        var scale = pair[0];
        var hash = pair[1];
    ...
    key: 'allShapesRelatedTo',
    value: function allShapesRelatedTo(lat, lng) {
      var _this6 = this;

      var scales = _.chain(this.__geoHashByScale).keys().map(function (scale) {
        return parseInt(scale);
      }).value();
      var largestScale = _.max(scales);

      var _Util$projectionCoord = Util.projectionCoords([lat, lng], largestScale),
          latIdx = _Util$projectionCoord.latIdx,
          lngIdx = _Util$projectionCoord.lngIdx;

      var shapes = [];
      scales.forEach(function (scale) {
        var bounds = Util.projectionIdxToBounds(latIdx, lngIdx, scale);
        console.log('bounds: ', bounds);
        GeoHash.forEachCell(bounds, scale, function (i, j) {
          shapes = shapes.concat(_this6.getHash(scale).getByIdx(i, j));
        });
      });
    ...
    key: 'allStillRendered',
    value: function allStillRendered() {
      return _.chain(this.allShapes()).filter(function (shape) {
    ...
    key: '_flashBounds',
    value: function _flashBounds(bounds) {
      var north = bounds.north,
          south = bounds.south,
          east = bounds.east,
          west = bounds.west;

      var path = [[north, west], [north, east], [south, east], [south, west]];

      path = _.map(path, function (p) {
        return Util.fromPointArrayToLatLngLiteral(p);
      });

      var options = Object.assign({}, Constants.DEFAULT_SHAPE_OPTIONS, {
        path: path,
        fillOpacity: 0,
        strokeColor: '#ff0000',
        strokeOpacity: 0.5
...
},{"./constants":1,"./geohash":2,"./util":6,"q":12,"underscore":13}],6:[function(require,module,exports){
'use strict';
...
function projectionCoords(coords, scale) {
  ...
  var _cellSizesFromScale = cellSizesFromScale(scale),
      latSize = _cellSizesFromScale.latSize,
      lngSize = _cellSizesFromScale.lngSize;
...
function projectionIdxToBounds(latIdx, lngIdx, scale) {
  var _cellSizesFromScale2 = cellSizesFromScale(scale),
      latSize = _cellSizesFromScale2.latSize,
      lngSize = _cellSizesFromScale2.lngSize;

  var south = latIdx * latSize + fudgeFactor;
  var north = (latIdx + 1) * latSize + fudgeFactor;

  var west = lngIdx * lngSize + fudgeFactor;
  var east = (lngIdx + 1) * lngSize + fudgeFactor;
...
module.exports = {
  ...
  projectionIdxToBounds: projectionIdxToBounds,
  normalizeLng: normalizeLng,
  normalizeLat: normalizeLat,

  combinedBounds: function combinedBounds(bounds1, bounds2) {
    return {
  ...
  makeChunks: function makeChunks(arr, size) {
    var chunks = {};
  ...
  areFloatsEqual: function areFloatsEqual(a, b) {
    return Math.abs(a - b) < .000001;
  },

  chainPromises: function chainPromises(promises) {
  ...
  fromPointArrayToLatLngLiteral: function fromPointArrayToLatLngLiteral(point) {
    return {
      lat: point[0],
      lng: point[1]
    };
  ...
  projectLat: function projectLat(lat, scale) {
    if (!lat || !scale) {
      throw 'missing';
    }

    var _cellSizesFromScale3 = cellSizesFromScale(scale),
        latSize = _cellSizesFromScale3.latSize;

    return Math.floor((lat - fudgeFactor) / latSize);
  },

  projectLng: function projectLng(lng, scale) {
    if (!lng || !scale) {
      throw 'missing';
    }

    var _cellSizesFromScale4 = cellSizesFromScale(scale),
        lngSize = _cellSizesFromScale4.lngSize;

    return Math.floor((lng - fudgeFactor) / lngSize);
  }
};

},{"./constants":1,"q":12,"underscore":13}],7:[function(require,module,exports){
'use strict'
...
},{"base64-js":7,"ieee754":10}],9:[function(require,module,exports){
(function (Buffer){
